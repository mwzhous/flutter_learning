...
class Chap0311 extends StatelessWidget {
  const Chap0311({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('11: 通过读取图片颜色为布局区域着色'),
      ),
      body: const Material(
        child: HomePage(),
      ),
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    loadImageFromAssets('assets/images/me.jpg').then((value) {
      setState(() {
        image = value;
      });
    });
