...
class Chap0312 extends StatelessWidget {
  const Chap0312({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('12: 行列为奇数时，显示颜色'),
      ),
      body: const Material(
        child: HomePage(),
      ),
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    loadImageFromAssets('assets/images/Android_Studio_Alt.png').then((value) {
      setState(() {
        image = value;
      });
    });
