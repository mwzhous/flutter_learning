...
import 'package:bottom_nav/safe/r_dotted_line_border.dart';
...
class DemoVerity extends StatefulWidget {
  ...
  const DemoVerity({Key? key, required this.lister}) : super(key: key);

  @override
  DemoVerityState createState() => DemoVerityState();
}

class DemoVerityState extends State<DemoVerity> with TickerProviderStateMixin {
  /// 半径
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      answerAnimationController.repeat(reverse: true);
    });
  }
            ...
            child: GestureDetector(
              onPanUpdate: (DragUpdateDetails details) {
                ...
                if ((distance < rDistance) && !success) {
                  ...
                  answerAnimationController.stop();
                  answerAnimationController.animateTo(1.0,
                ...
                if ((distance >= rDistance) && success) {
                  success = false;
                  if (!answerAnimationController.isAnimating) {
                    answerAnimationController.repeat(reverse: true);
                  }
                }
